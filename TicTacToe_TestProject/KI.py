...
def can_make_three(position, player_symbol, gamestate):
    ...
    which symbole to check for                        :param player_symbol:
    List to check                                     :param gamestate:
    retrun which position to use                      :return:
...
def find_best_choise(player_symbol, opponent_symbol, gamestate):
    ...
    which symbole uses the player/cpu                 :param player_symbol:
    which symbole uses the Opponent                   :param opponent_symbol::
    List to check                                     :param gamestate:
    retrun which position to use                      :return:
    ...
    # is somehow a cpu win possible? First interesst!
    for i in range(von, bis, random_value):
        if can_make_three(i, player_symbol, gamestate):
            print ("cpu win with: " + str(i+1))
            return i + 1

    # Somehow Opp win possible? Needed to be blocked is second interesst
    for i in range(von, bis, random_value):
