...
class KIPlayer:
    ...
    def check_player_win(self, symbol):
        ...
        for KI_player_choice in range(temp_board.board_size_x):
            # drop a stone in virtual board copy!
            player_valid_move, player_last_played_position = temp_board.drop_stone(
                symbol, KI_player_choice
            )
            if player_valid_move:
                # check if KI player would win with this stone
                player_win_with = temp_board.check_win(
                    symbol, player_last_played_position, 4
                )
    ...
    def player_looks_one_turn_ahead(self):
        ...
        # copy of current board!
        temp_board = copy.deepcopy(self.current_board)
        boardsize = temp_board.board_size_x
        ...
        for column in range(boardsize):
            ...
            for choice in range(boardsize):
                ...
                ) = sub_temp_board.drop_stone(self.enemy_symbol, choice)
                if enemy_valid_move:
                    # check if ENEMY player would win with his next stone!!
                    enemy_win_with = sub_temp_board.check_win(
                        self.enemy_symbol, enemy_last_played_position, 4
                    )
    ...
    def play_best_option(self, only_choose_from_this_list: tuple) -> tuple:
        ...
        for lenght_of_connecting_stones in range(3,0,-1):
            options_to_choose_from = []
            ...
            for column in only_choose_from_this_list:
                ...
                temp_board = copy.deepcopy(self.current_board)
                # drop a stone in virtual board copy!
                valid_move, player_last_played_position = temp_board.drop_stone(self.player_symbol, column)
                if not valid_move:
                    continue
...
############################################################################################################################
    def old_play_best_option(self, only_choose_from_this_list: tuple) -> tuple:
        """method to check for best stone to play"""
        # copy of current board!
        temp_board = copy.deepcopy(self.current_board)
        boardsize = temp_board.board_size_x
            ...
            for column in only_choose_from_this_list:
                ...
                for choice in range(boardsize):
                    ...
                    (
                        player_valid_move,
                        player_last_played_position,
                    ) = sub_temp_board.drop_stone(self.player_symbol, choice)
                    if player_valid_move:
                        # check if KI player would win with his next stone!!
                        player_win_with = sub_temp_board.check_win(
                            self.player_symbol,
                            player_last_played_position,
                            lenght_of_connecting_stones,
                        )
